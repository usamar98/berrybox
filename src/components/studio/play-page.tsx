...
export function PlayPage({ templateId }: { templateId: TemplateId }) {
  ...
  return <PageShell><section className="studio-container py-10"><Link href="/ai-3d-scene-generator" className="studio-back"><ArrowLeft size={16} /> Back to scene generator</Link><div className="my-7 flex flex-wrap items-center justify-between gap-4"><div><p className="studio-eyebrow">PLAYABLE TEMPLATE</p><h1 className="mt-3 text-3xl font-semibold">{template.title}</h1></div><Link href={"/editor?template=" + templateId + "&new=1"} className="studio-primary">Make it yours <ArrowRight size={16} /></Link></div><GamePreview config={template.config} /></section></PageShell>;
}
