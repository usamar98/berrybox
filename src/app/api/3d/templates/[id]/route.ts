import { FalProviderError, getTemplateTask, resolveTemplateModel, templateGenerationAvailable } from "@/lib/3d/fal";
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const model = resolveTemplateModel(new URL(request.url).searchParams.get("model") || undefined);
  if (!model) return Response.json({ error: "The selected fal model is not enabled for this deployment." }, { status: 400 });

  try {
    const task = await getTemplateTask(id, model, request.signal);
